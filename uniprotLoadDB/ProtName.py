...
class ProtName:
    ...
    def insertDB(self, curDB):
        ...
        curDB.prepare(" SELECT prot_name_id" \
                        ...
                        +"AND name_type = :name_type")##Fin prepare
        curDB.execute(None,{'prot_name':self._name,'name_kind':self._name_kind,'name_type':self._name_type})##Fin execute
            ####          ####
            #### FIN TODO ####
        ...
        if raw != None:
            ...
            if ProtName.DEBUG_INSERT_DB:
                ...
                curDB.prepare("INSERT INTO protein_names" \
                              ...
                              +"VALUES"\
                              +"(seq_prot_names.NEXTVAL,:prot_name,:name_kind,:name_type)"
                              +"RETURNING prot_name_id INTO :id")##fin prepare INSERT
                              
                curDB.execute(None,{'prot_name':self._name,'name_kind':self._name_kind,'name_type':self._name_type,'id':idG})
                ####          ####
                prot_name_id = idG.getvalue()
                #### FIN TODO ####
        return prot_name_id 